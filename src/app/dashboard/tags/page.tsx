...
export default function TagsPage() {
  ...
  const filtered = useMemo(() => {
    const arr = tags.filter((t) => t.name.toLowerCase().includes(search.toLowerCase()));
  ...
  async function load() {
    setLoading(true);
    try { const r = await blogService.manage.tags.list(); setTags(r.data); }
  ...
  function openCreate() { setName(''); setModal('create'); }
  function openEdit(t: BlogTag) { setName(t.name); setModal(t); }
  ...
  async function handleSave() {
    ...
    try {
      if (modal === 'create') {
        await blogService.manage.tags.create({ name });
        toast.success('Tag created');
      } else if (modal && typeof modal !== 'string') {
        await blogService.manage.tags.update(modal.id, { name });
  ...
  async function handleDelete(id: string) {
    try { await blogService.manage.tags.delete(id); toast.success('Deleted'); load(); }
      ...
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <h1 className="text-2xl font-bold text-[#0A1628]">Tags</h1>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-3 mb-4">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search tags..." className="w-full border border-[#E8EDF2] rounded-lg pl-9 pr-3 py-2 text-sm text-[#0A1628] focus:outline-none focus:ring-2 focus:ring-[#4F46E5]/30" />
          </div>
                  ...
                  <tr key={t.id} className="hover:bg-[#F8FAFC]">
                    ...
                    <td className="px-5 py-3">
                      <div className="flex gap-3 justify-end">
                        <button onClick={() => openEdit(t)} className="text-xs text-[#4F46E5] hover:underline font-medium">Edit</button>
                        <button onClick={() => setDeleteConfirm(t)} className="text-xs text-red-400 hover:underline font-medium">Delete</button>
                      </div>
                    </td>
                  </tr>
                ))}
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            ...
            {deleteConfirm._count?.posts ? (
              <p className="text-xs text-red-500 font-medium">
                This tag is used in {deleteConfirm._count.posts} post{deleteConfirm._count.posts > 1 ? 's' : ''}. Tags will be removed from those posts.
              </p>
            ) : null}
      ...
      {modal !== null && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            ...
            <div className="flex gap-2 mt-5">
              <button onClick={() => setModal(null)} className="flex-1 border border-[#E8EDF2] text-[#4A4A6A] font-medium text-sm py-2.5 rounded-lg">Cancel</button>
