...
export default function CategoryArchivePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      blogService.posts.list({ category: slug, limit: '50' }),
    ])
      .then(([catRes, postRes]) => {
        const found = catRes.data.find((c: BlogCategory) => c.slug === slug);
        ...
        <section className="bg-gradient-to-br from-[#0A1628] to-[#1A1A2E] py-16 px-4">
          <div className="max-w-7xl mx-auto text-center">
            ...
            {category?.description && (
              <p className="text-[#C0C9D5] text-lg max-w-2xl mx-auto">{category.description}</p>
            )}
            <div className="mt-4">
        ...
        <section className="max-w-7xl mx-auto px-4 py-12">
          {loading ? (
            ...
            <div className="text-center py-20">
              <p className="text-[#C0C9D5] text-lg">No articles found in this category.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
