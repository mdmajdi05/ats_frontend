    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="flex flex-wrap gap-1.5 mb-5">
        ...
        {Object.entries(TYPE_CONFIG).map(([key, cfg]) => {
          ...
          return (
            <button key={key} onClick={() => setFilter(key)} className={cn('px-3 py-1.5 rounded-lg text-xs font-semibold transition-all', filter === key ? 'bg-navy text-white' : 'bg-white border border-silver text-text-muted hover:border-navy/30')}>
              {cfg.label} ({count})
            </button>
          );
        })}
